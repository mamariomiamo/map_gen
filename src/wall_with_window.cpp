...
void clickCallback(const geometry_msgs::PoseStamped& msg) {
  ...
  for (double l1 = 0.0; l1 <= len + 1e-3; l1 += resolution_) {
    Eigen::Vector3d tmp1 = p1 + l1 * dir1;
    for (double l2 = -len2_; l2 <= len2_ + 1e-3; l2 += resolution_) {
      Eigen::Vector3d tmp2 = tmp1 + l2 * dir2;
      for (double h = -0.5; h < wall_height; h += resolution_) {
        pt_random.x = tmp2[0];
        pt_random.y = tmp2[1];
        pt_random.z = h;
  ...
  for (int i = 0; i < map_cloud_->points.size(); i++)
  {
...
int main(int argc, char** argv) {
  ...
  n.param("map/radius", radius, 0.5);
  n.param("map/wall_height", wall_height, 5.0);
  n.param("map/window_height", window_height, 5.0);
